...
fun buildVibrator(context: Context) {
    notificationBuilderPattern1 = NotificationCompat.Builder(context, MainActivity.CHANNEL_ID_1)
    ...
    notificationBuilderPattern2 = NotificationCompat.Builder(context, MainActivity.CHANNEL_ID_2)
        .setSmallIcon(R.drawable.ic_launcher_foreground)
...
fun alertDriver(context: Context) {
    if(speed >= 55.0) {
        alert01(context)
    } else if(speed >= 10.0) {
        alert02(context)
...
private fun alert01(context: Context) {
    ...
    with(NotificationManagerCompat.from(context)) {
        ...
        notify(MainActivity.NOTIFICATION_ID, notificationBuilderPattern1.build())
    }
...
private fun alert02(context: Context) {
    ...
    with(NotificationManagerCompat.from(context)) {
        // notificationId is a unique int for each notification that you must define
        notify(MainActivity.NOTIFICATION_ID, notificationBuilderPattern2.build())
    }
